...
def compute_dct_low_frequency_feature(flat_update, low_freq_ratio=0.5, max_features=128):
    """Return a compact, fixed-size descriptor of low-frequency DCT energy."""
    vec = flat_update.detach().float().cpu().view(-1)
    max_features = max(1, int(max_features))
    if vec.numel() == 0:
        return np.zeros(max_features, dtype=float)

    side = int(np.ceil(np.sqrt(vec.numel())))
    padded = torch.zeros(side * side, dtype=torch.float32)
    padded[:vec.numel()] = vec
    matrix = padded.reshape(side, side)
    block_size = max(1, min(side, int(np.ceil(side * low_freq_ratio))))

    if scipy_fft is not None:
        transformed = scipy_fft.dctn(matrix.numpy(), norm='ortho')
        low = np.abs(transformed[:block_size, :block_size]).reshape(-1)
    else:
        transformed = torch_dct_2d(matrix).numpy()
        low = np.abs(transformed[:block_size, :block_size]).reshape(-1)

    if len(low) <= max_features:
        pooled = np.pad(low, (0, max_features - len(low)), mode='constant')
    else:
        edges = np.linspace(0, len(low), max_features + 1, dtype=int)
        pooled = np.asarray([
            low[edges[i]:edges[i + 1]].mean()
            if edges[i] < edges[i + 1] else 0.0
            for i in range(max_features)
        ], dtype=float)
    return np.asarray(pooled, dtype=float)


def phase2_view_deviation(value, references, mad_k=1.5, eps=1e-8):
    """Compute the paper's coordinate-wise median/MAD interval deviation."""
    value = np.asarray(value, dtype=float).reshape(-1)
    references = np.asarray(references, dtype=float)
    if references.size == 0:
        return 0.0, np.zeros_like(value)
    ...
    lower = median - float(mad_k) * mad
    upper = median + float(mad_k) * mad
    deviation = np.zeros_like(value, dtype=float)

    lower_mask = value <= lower
    upper_mask = value >= upper
    lower_denominator = np.maximum(np.abs(lower), eps)
    upper_denominator = np.maximum(np.abs(upper), eps)
    deviation[lower_mask] = (lower[lower_mask] - value[lower_mask]) / lower_denominator[lower_mask]
    deviation[upper_mask] = (value[upper_mask] - upper[upper_mask]) / upper_denominator[upper_mask]
    return float(np.mean(np.maximum(deviation, 0.0))), deviation


def compute_phase2_ecdf_risks(feature_views, reference_queue, weights,
                               mad_k=1.5, risk_quantile=0.90, eps=1e-8):
    """Fuse raw deviations first, then convert the fused score with an ECDF."""
    ...
    normalized_weights /= max(float(normalized_weights.sum()), eps)

    if reference_queue:
        reference_views = {
            name: np.stack([
                np.asarray(entry['views'][name], dtype=float)
                for entry in reference_queue
            ], axis=0)
            for name in names
        }
    else:
        reference_views = {
            name: np.asarray(feature_views[name], dtype=float)
            for name in names
        }

    current_component_deviations = {
        name: np.zeros(num_clients, dtype=float) for name in names
    }
    for name in names:
        references = reference_views[name]
        for cid in range(num_clients):
            current_component_deviations[name][cid], _ = phase2_view_deviation(
                feature_views[name][cid], references, mad_k=mad_k, eps=eps,
            )

    current_matrix = np.stack(
        [current_component_deviations[name] for name in names], axis=1
    )
    current_deviations = current_matrix @ normalized_weights

    reference_count = reference_views[names[0]].shape[0]
    benign_deviations = []
    for ref_index in range(reference_count):
        leave_one_out = {}
        for name in names:
            values = reference_views[name]
            leave_one_out[name] = (
                np.delete(values, ref_index, axis=0)
                if values.shape[0] > 2 else values
            )
        component_scores = []
        for name in names:
            score, _ = phase2_view_deviation(
                reference_views[name][ref_index],
                leave_one_out[name],
                mad_k=mad_k,
                eps=eps,
            )
            component_scores.append(score)
        benign_deviations.append(float(np.dot(normalized_weights, component_scores)))

    benign_deviations = np.asarray(benign_deviations, dtype=float)
    if benign_deviations.size == 0:
        benign_deviations = np.zeros(1, dtype=float)
    sorted_benign = np.sort(benign_deviations)
    # Use a mid-rank for ties. Otherwise a degenerate benign distribution
    # (e.g., all zero deviations during cold start) would assign risk 1.0
    # to every benign value that is exactly equal to the reference maximum.
    tie_tolerance = 1e-10
    lower_rank = np.searchsorted(
        sorted_benign, current_deviations - tie_tolerance, side='left'
    )
    upper_rank = np.searchsorted(
        sorted_benign, current_deviations + tie_tolerance, side='right'
    )
    risks = (lower_rank + upper_rank) / (2.0 * len(sorted_benign))
    threshold = float(risk_quantile)
    return (
        current_deviations,
        risks.astype(float),
        threshold,
...
def compute_phase1_direction_deviations(updates, eps=1e-12):
    """Compute the angular deviation from the preceding client update."""
    deviations = np.zeros(len(updates), dtype=float)
    for index in range(1, len(updates)):
        current = updates[index].float().view(-1)
        previous = updates[index - 1].float().view(-1)
        current_norm = torch.norm(current, p=2)
        previous_norm = torch.norm(previous, p=2)
        if current_norm.item() <= eps or previous_norm.item() <= eps:
            continue
        cosine = torch.dot(current, previous) / (current_norm * previous_norm)
        cosine = torch.clamp(cosine, min=-1.0, max=1.0)
        deviations[index] = float(torch.acos(cosine).item())
    return deviations


def compute_phase1_reference_scores(metrics, reference_queue, mad_k=1.4829, eps=1e-8):
    """Apply the paper's median/MAD normalization and compute alpha values."""
    values = np.asarray(metrics, dtype=float)
    if values.ndim != 2:
        raise ValueError("Phase-I metrics must have shape [num_clients, num_indicators].")

    if reference_queue:
        references = np.stack([np.asarray(item, dtype=float) for item in reference_queue])
    else:
        references = values
    if references.ndim == 1:
        references = references.reshape(-1, values.shape[1])

    median = np.median(references, axis=0)
    mad = np.median(np.abs(references - median), axis=0)
    denominator = float(mad_k) * mad + eps
    z_components = np.maximum(0.0, values - median) / denominator
    deviation = np.mean(z_components, axis=1)
    alpha = np.exp(-0.5 * np.square(deviation))
    return z_components, deviation, alpha, median, mad


def dominant_dbscan_partition(clusters):
    """Return the dominant DBSCAN cluster and clients outside it."""
    labels = np.asarray(clusters, dtype=int)
    valid = labels[labels >= 0]
    if valid.size == 0:
        # Without a dense cluster, do not exclude clients on weak evidence.
        return [], list(range(len(labels))), None

    unique, counts = np.unique(valid, return_counts=True)
    dominant = int(unique[np.argmax(counts)])
    unseen = np.where(labels != dominant)[0].astype(int).tolist()
    others = np.where(labels == dominant)[0].astype(int).tolist()
    return unseen, others, dominant
...
def compute_lfd_scores_against_reference(feature_distributions, reference_distributions, eps=1e-12):
    """Compute LFD against the median frequency distribution in the benign queue."""
    distributions = np.asarray(feature_distributions, dtype=float)
    references = np.asarray(reference_distributions, dtype=float)
    if references.size == 0:
        return compute_lfd_scores(distributions, eps=eps)
    if references.ndim == 1:
        references = references.reshape(1, -1)

    ref = np.median(references, axis=0)
    ref = np.maximum(ref, eps)
    ref = ref / np.sum(ref)
    scores = []
    for p in distributions:
...
def main(config_path, args):
    ...
    # Use the YAML value unless the caller explicitly overrides it.
    cfg['num_clients'] = args.num_clients if args.num_clients is not None else cfg['num_clients']
    ...
    phase1_reference_queue = []
    phase1_lfd_reference_queue = []
    phase1_reference_size = max(1, cfg['num_clients'] // 2)
    phase2_benign_queue = []
    phase2_reference_size = max(1, int(args.phase2_history_size))
    ...
    for ep in range(cfg['epochs']):
        ...
        # ================= Paper-aligned RobustSL path =================
        # The original implementation remains available with --legacy-logic.
        # The default path below follows the paper literally and trains each
        # client once per round so Phase I and Phase II inspect the same update.
        if not args.legacy_logic:
            client_features = []
            client_feature_maps = []
            phase1_updates = []
            ...
            for i in range(cfg['num_clients']):
                prepare_client_from_global(
                    clients[i], server, global_flat, global_server_buffers
                )
                stats = clients[i].run_batches(
                    server,
                ...
                update = (
                    get_flat_weights(clients[i].head, server.model, clients[i].tail)
                    .detach().cpu()
                    - global_flat_cpu
                )
                if args.enable_attack and i in malicious_clients:
                    update = update * args.attack_scale
                phase1_updates.append(update.clone())

                _, backbone_delta, _ = split_flat_update(
                    clients[i].head, server.model, clients[i].tail, update
                )
                dct_raw = compute_dct_low_frequency_score(
                    backbone_delta,
                    low_freq_ratio=args.phase2_dct_ratio,
                )
                dct_feature = compute_dct_low_frequency_feature(
                    backbone_delta,
                    low_freq_ratio=args.phase2_dct_ratio,
                    max_features=args.phase2_dct_feature_dim,
                )
                backbone_norm = float(torch.norm(backbone_delta.float(), p=2).item())
                l2_feature = np.asarray([backbone_norm], dtype=float)
                ...
                current_round_dct_scores.append(dct_raw)
                current_round_update_norms.append(backbone_norm)
                ...
                print(
                    ...
                    f"DCT-low={dct_raw:.4f} | ||Delta M||={backbone_norm:.4f}"
                )
            ...
            # Phase I follows the paper's three indicators exactly:
            # phi^1 = 1 - f_spa, phi^2 = arccos(cosine to client i-1), phi^3 = LFD.
            sparsity_values = np.asarray(
                [compute_gradient_spa(update) for update in phase1_updates],
                dtype=float,
            )
            sparsity_deviations = 1.0 - sparsity_values
            direction_deviations = compute_phase1_direction_deviations(phase1_updates)
            lfd_distributions = [
                haar_frequency_distribution(feature_map)
                for feature_map in client_feature_maps
            ]
            if phase1_lfd_reference_queue:
                lfd_scores = compute_lfd_scores_against_reference(
                    lfd_distributions,
                    np.stack(phase1_lfd_reference_queue),
                )
            else:
                # Cold start: no verified benign queue exists yet, so use the
                # current-round median only to initialize the first queue.
                lfd_scores = compute_lfd_scores(lfd_distributions)
            phase1_metrics_np = np.column_stack(
                (sparsity_deviations, direction_deviations, lfd_scores)
            )
            ...
            predicted_unseen, others_ids, dominant_cluster = dominant_dbscan_partition(
                dbscan_clusters
            )

            # The first round seeds the benign reference queue from the
            # dominant DBSCAN cluster; later rounds use verified clients only.
            if not phase1_reference_queue:
                seed_ids = others_ids if others_ids else list(range(cfg['num_clients']))
                for cid in seed_ids:
                    append_limited(
                        phase1_reference_queue,
                        phase1_metrics_np[cid].copy(),
                        phase1_reference_size,
                    )
            if not phase1_lfd_reference_queue:
                seed_ids = others_ids if others_ids else list(range(cfg['num_clients']))
                for cid in seed_ids:
                    append_limited(
                        phase1_lfd_reference_queue,
                        np.asarray(lfd_distributions[cid], dtype=float).copy(),
                        phase1_reference_size,
                    )
            phase1_z, unseen_scores, phase1_alphas, phase1_median, phase1_mad = (
                compute_phase1_reference_scores(
                    phase1_metrics_np,
                    phase1_reference_queue,
                    mad_k=args.phase1_mad_k,
                )
            )
            phase1_clusters = np.asarray(dbscan_clusters, dtype=int)

            print(
                "\n  [Phase 1] --- DBSCAN on gradient sparsity / direction / LFD ---"
            )
            print(
                f"  [Phase 1] Dominant cluster={dominant_cluster} | "
                f"reference_queue={len(phase1_reference_queue)}/{phase1_reference_size}"
            )
            print(
                f"  [Phase 1] Predicted Unseen IDs: {predicted_unseen} | "
                f"Others IDs: {others_ids}"
            )
            print(
                f"  [Phase 1] Reference median="
                f"{np.asarray(phase1_median).round(5).tolist()} | "
                f"MAD={np.asarray(phase1_mad).round(5).tolist()}"
            )
            for i in range(cfg['num_clients']):
                pred_label = 'Unseen' if i in predicted_unseen else 'Others'
                print(
                    f"    Client {i:02d} | {pred_label:<6} | "
                    f"phi1={sparsity_deviations[i]:.5f} | "
                    f"phi2={direction_deviations[i]:.5f} | "
                    f"phi3={lfd_scores[i]:.5f} | "
                    f"Z={unseen_scores[i]:.5f} | alpha={phase1_alphas[i]:.5f}"
                )
            ...
            print(
                f"  >>> [Phase 1] Unseen recall: {recall * 100:.1f}% | "
                f"FPR: {fpr * 100:.1f}%"
            )
            ...
            if not phase2_benign_queue:
                seed_ids = others_ids if len(others_ids) >= 2 else list(range(cfg['num_clients']))
                for cid in seed_ids:
                    append_limited(
                        phase2_benign_queue,
                        {
                            'flat': current_round_updates[cid].clone(),
                            'views': {
                                name: np.asarray(values[cid], dtype=float).copy()
                                for name, values in feature_views.items()
                            },
                        },
                        phase2_reference_size,
                    )
            ...
            if args.enable_defense:
                (
                    ...
                    phase2_risks,
                    phase2_threshold,
                    phase2_component_deviations,
                    benign_deviation_scores,
                ) = compute_phase2_ecdf_risks(
                    feature_views,
                    phase2_benign_queue,
                    {
                ...
                phase2_backdoor_ids = [
                    ...
                    if phase2_risks[cid] > phase2_threshold
                ]
            else:
                phase2_deviations = np.zeros(cfg['num_clients'], dtype=float)
                phase2_risks = np.zeros(cfg['num_clients'], dtype=float)
                phase2_threshold = args.phase2_risk_quantile
                phase2_component_deviations = {
                    name: np.zeros(cfg['num_clients'], dtype=float)
                    for name in feature_views
                }
                benign_deviation_scores = np.zeros(1, dtype=float)
                phase2_backdoor_ids = []

            phase2_benign_ids = [
                cid for cid in range(cfg['num_clients'])
                if cid not in phase2_backdoor_ids
            ]
            print(
                f"\n  [Phase 2] --- DCT/L2/Gram deviation fusion + ECDF ---"
            )
            print(
                f"  [Phase 2] Reference queue="
                f"{len(phase2_benign_queue)}/{phase2_reference_size} | "
                f"MAD-k={args.phase2_mad_k:.4f} | "
                f"risk threshold={phase2_threshold:.4f}"
            )
            print(
                f"  [Phase 2] Backdoor IDs: {phase2_backdoor_ids} | "
                f"Benign IDs: {phase2_benign_ids}"
            )

            reference_ids = phase2_benign_ids
            ...
            if not reference_ids:
                reference_ids = list(range(cfg['num_clients']))
            reference_stack = torch.stack(
                [current_round_updates[cid] for cid in reference_ids]
            )
            reference_update = reference_stack.mean(dim=0)
            reference_server_buffers = average_model_buffers(
                [current_round_server_buffers[cid] for cid in reference_ids]
            )
            ...
            for cid in range(cfg['num_clients']):
                ...
                if is_backdoor and not is_unseen:
                    filtered_backdoor_ids.append(cid)
                    action = 'FILTER: others+backdoor'
                    blend_coeff = 0.0
                    weight = 0.0
                elif is_backdoor and is_unseen:
                    # Delta g* = alpha * Delta g_orig +
                    #          (1-alpha) * Delta g_new.
                    blend_coeff = float(phase1_alphas[cid])
                    update_star = (
                        blend_coeff * current_round_updates[cid]
                        + (1.0 - blend_coeff) * reference_update
                    )
                    buffer_star = blend_model_buffers(
                        current_round_server_buffers[cid],
                        reference_server_buffers,
                        blend_coeff,
                    )
                    ...
                    action = 'BLEND: unseen+backdoor'
                    weight = 1.0
                else:
                    update_star = current_round_updates[cid]
                    accepted_updates.append(update_star)
                    accepted_weights.append(1.0)
                    accepted_server_buffers.append(current_round_server_buffers[cid])
                    blend_coeff = 1.0
                    weight = 1.0
                    if is_unseen:
                        unseen_benign_ids.append(cid)
                        action = 'KEEP: unseen+benign'
                    else:
                        benign_kept_ids.append(cid)
                        action = 'KEEP: others+benign'

                print(
                    f"    Client {cid:02d} | Phase1={'unseen' if is_unseen else 'others':<6} | "
                    f"Phase2={'backdoor' if is_backdoor else 'benign':<8} | "
                    f"DCT/L2/Gram="
                    f"{phase2_component_deviations['dct'][cid]:.5f}/"
                    f"{phase2_component_deviations['l2'][cid]:.5f}/"
                    f"{phase2_component_deviations['gram'][cid]:.5f} | "
                    f"D={phase2_deviations[cid]:.5f} | R={phase2_risks[cid]:.5f} | "
                    f"alpha={phase1_alphas[cid]:.5f} | blend={blend_coeff:.3f} | "
                    f"w={weight:.3f} | {action}"
                )

            # Only Phase-II benign clients enter the reference queues. This
            # prevents an uncorrected suspicious update from contaminating them.
            queue_feature_ids = phase2_benign_ids if args.enable_defense else list(range(cfg['num_clients']))
            for cid in queue_feature_ids:
                append_limited(
                    phase1_reference_queue,
                    phase1_metrics_np[cid].copy(),
                    phase1_reference_size,
            ...
            if accepted_updates:
                ...
                agg_delta = (
                    stacked_updates
                    * weights_t.view(-1, *([1] * (stacked_updates.dim() - 1)))
                ).sum(dim=0)
                agg_delta = (
                    agg_delta / max(float(weights_t.sum().item()), 1e-8)
                ).to(device)
                aggregated_server_buffers = average_model_buffers(
                    accepted_server_buffers,
                    accepted_weights,
                )
            else:
                print('  [Warning] No accepted updates; the global model is unchanged.')
            ...
            if args.root_calib_steps > 0:
                ...
                global_flat = get_flat_weights(
                    head0, server.model, tail0
                ).detach()
                global_server_buffers = get_model_buffers(server.model)
                print(
                    f"  [RootCalib] steps={args.root_calib_steps} "
                    f"loss={root_calib_loss:.4f}"
                )

            acc = eval_clean_accuracy(
                head0, tail0, server, test_loader, device=device
            )
            china_acc = eval_unseen_accuracy(
                head0, tail0, server, china_test_loader, device=device
            )
            if args.enable_attack and args.attack_type == 'trigger':
                asr_strict = eval_asr_strict(
                    head0,
                    tail0,
                    server,
                    test_loader,
                ...
                asr_msg = 'N/A'
            print(
                f"\n  [Eval] MA: {acc:.4f} | Unseen Accuracy: {china_acc:.4f} | "
                f"ASR: {asr_msg} | "
                f"Defense={'ON' if args.enable_defense else 'OFF'}"
            )
            continue
...
if __name__ == '__main__':
    ...
    ap.add_argument('--phase1-candidate-multiplier', type=float, default=1.5, help='Legacy Phase 1 参数；论文对齐路径由 DBSCAN 主簇直接确定 unseen')
    ap.add_argument('--phase1-mad-k', type=float, default=1.4829, help='Phase 1 median/MAD 归一化中的比例因子')
    ...
    ap.add_argument('--phase2-dct-feature-dim', type=int, default=128, help='Phase 2 DCT 低频特征的固定压缩维度')
    ...
    ap.add_argument('--legacy-logic', action='store_true', help='使用修改前的 RobustSL 逻辑，仅用于回滚对照')
