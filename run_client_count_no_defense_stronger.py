#!/usr/bin/env python3
"""Run stronger no-defense GTSRB experiments for client-count comparison."""
...
RUN_ROOT = ROOT / 'client_count_no_defense_stronger_runs'
SUMMARY_CSV = ROOT / 'client_count_no_defense_results_stronger.csv'
SUMMARY_MD = ROOT / 'client_count_no_defense_results_stronger.md'
CLIENT_COUNTS = [20, 25, 30, 40]
...
def command_for(total: int) -> list[str]:
    return [
        ...
        '--unseen', '0.70',
        '--num-malicious', str(ratio_count(total, 0.25)),
        '--attack-type', 'trigger',
        '--poison-rate', '1.00',
        '--attack-scale', '5.0',
...
def write_results(rows: list[dict]) -> None:
    ...
    lines = [
        ...
        'Stronger attack setting with `--enable-defense` omitted. Only the 20-40 client-count cases are rerun.', '',
...
def run_one(total: int) -> dict:
    ...
    malicious_clients = ratio_count(total, 0.25)
